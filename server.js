...
// ========== НАСТРОЙКА СЕССИЙ (БЕЗ PostgreSQL STORE) ==========
app.use(session({
...
// Очистка старых сессий из БД (если таблица существует)
setInterval(async () => {
    try {
...
// ========== АДМИН ПАНЕЛЬ (УПРОЩЕННАЯ - БЕЗ ПРОВЕРКИ ПАРОЛЯ) ==========
...
// ========== КОНВЕРТЕР ФАЙЛОВ ==========
const imageFormats = ['jpg', 'jpeg', 'png', 'webp', 'bmp', 'tiff', 'gif', 'ico', 'avif'];
...
async function convertImage(inputPath, outputPath, targetFormat) {
    return sharp(inputPath).toFormat(targetFormat).toFile(outputPath);
}
...
async function convertTxtToDocx(inputPath, outputPath) {
    return new Promise(async (resolve, reject) => {
        try {
            const text = await fs.promises.readFile(inputPath, 'utf8');
...
app.post('/api/convert/batch', requireAuth, uploadMultiple.array('files', 10), async (req, res) => {
    let tempFiles = [];
    ...
    try {
        ...
        if (!targetFormat) {
            return res.status(400).json({ error: 'Не указан целевой формат' });
        }
        ...
        for (const file of files) {
            ...
            const baseName = getBaseFilename(file.originalname);
            ...
            try {
                ...
                if (isImage && imageFormats.includes(targetFormat)) {
                ...
                } else if (isTxt && targetFormat === 'pdf') {
                    await convertTxtToPdf(inputPath, outputPath);
                    results.push({ original: file.originalname, converted: outputFilename, success: true });
                } else if (isTxt && targetFormat === 'docx') {
                    ...
                    throw new Error(`Конвертация из ${mimeType} в ${targetFormat} не поддерживается`);
                }
            } catch (err) {
                errors.push({ original: file.originalname, error: err.message, success: false });
        ...
        await new Promise((resolve) => {
            db.get('SELECT COUNT(*) as count FROM admins', [], (err, result) => {
                if (!err && result && parseInt(result.count) === 0) {
                    ...
                    bcrypt.hash(process.env.ADMIN_PASSWORD || 'linksnap2026secure', 10, (err, hash) => {
                        if (!err) {
